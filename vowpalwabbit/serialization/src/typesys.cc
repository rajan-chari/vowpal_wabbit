...
// only needed to root type_builder into a .cc file, so that it can 
// propertly participate in syntax highlighting and error-squiggles
...
namespace typesys {
  ...
  type_registry::type_registry() : _types(), types(&_types)
  {
    ...
    max_builtin = static_cast<typesys::type_number>(this->_types.size() - 1);
                  // This cast is safe assuming we have < 4 billion types. Seems like a safe assumption for now.
                  // type_number is only valid at runtime anyways, so we are not really even saving much by making
                  // it uint32_t
  }
}